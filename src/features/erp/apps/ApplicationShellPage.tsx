        ...
        <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-4 px-4 md:px-6">
          <div className="min-w-0">
            <div className="flex items-center gap-2 text-xs text-slate-500">
              <Link to="/apps" className="hover:text-blue-700">Uygulamalar</Link>
              <ChevronRight className="h-3.5 w-3.5" />
              <span className="truncate font-medium text-slate-700">{app.title}</span>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-6 md:px-6 md:py-8">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">{app.description}</p>
              </div>
            </div>
            {app.permissionKey ? (
        ...
        <div className="grid gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
          <aside className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
            <p className="px-2 pb-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500">Modüller</p>
          ...
          <section className="min-w-0">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-slate-500">Mevcut ERP ekranlarına hızlı geçiş ve gelecekteki genişleme alanları.</p>
              </div>
            </div>

            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {app.modules.map((module) => (
                <Card key={`${module.title}-${module.route}`} className="rounded-lg border-slate-200 shadow-sm">
                  <CardContent className="flex h-full flex-col p-4">
                    ...
                    {module.permissionKey ? <p className="mt-3 text-xs text-slate-400">Yetki anahtarı hazır</p> : null}
