...
|---|---|---|---|---|---|---|---|---|---|
| 1A-1 | Suspected unbounded retry/checkpoint loop can re-fail a poisoned row every cron tick with zero backoff (suspected outage mechanism, 2026-08-24 Auth outage) | CRITICAL | per-record error path counts errors but has no invocation-level retry budget/circuit breaker; resume policy can re-enter same page repeatedly across invocations | `server/parasut/sync-base.ts`, `server/parasut/client.ts`, `supabase/functions/parasut-sync-run/index.ts`, `parasut.sync_runs` usage | bounded consecutive-error abort already exists (MAX_CONSECUTIVE_RESOURCE_ERRORS=5) — audit its interaction with resume; add per-run retry budget + exponential backoff metadata on repeated identical record failures; circuit-breaker state persisted in sync_runs; hard execution budget | OPEN | new vitest cases reproducing poison-row loop | LOW-MED (sync engine; currently paused so no prod exposure while testing) | git revert of engine commits |
| 1B-1 | No sync health observability; paused state invisible to operators; staleness drift undetectable while paused | HIGH | health only inferable from sync_runs rows; no machine-readable status surface | `supabase/functions/parasut-api`, new shared health module, admin/dashboard surface | persisted-health computation from sync_runs/staleness (pure module + tests), exposed via parasut-api `sync-status` extension or dedicated function section; machine-readable JSON | OPEN | pure-function unit tests | LOW | revert commit |
| 1C-1 | `send-quotation-email`: no auth, no captcha, CORS `*` → open SMTP relay abuse vector | CRITICAL | function written before hardening pass that contact form received | `supabase/functions/send-quotation-email/index.ts` (+ any frontend callers: legacy `features/quotation/hooks/useQuotationPDF.ts:178`) | port contact-email protections (origin allowlist, size/type limits) + ERP-user auth requirement (quotation send is an authenticated ERP action) + input validation + rate limiting via existing patterns; keep payload contract compatible with current sales quote flow if it calls this function | OPEN | abuse/security unit tests + legitimate-path test | MED (verify actual callers first — legacy caller may be orphaned) | revert commit |
...
| # | Issue | Severity | Root cause | Files affected | Remediation plan | Status | Tests | Regression risk | Rollback |
|---|---|---|---|---|---|---|---|---|---|
| 3-1 | Three lockfiles (npm/bun/deno), no `packageManager` pin; bun.lockb unused drift risk | MEDIUM | scaffold history; deno.lock partially legit (edge functions import maps) | `package.json`, `bun.lockb`, `deno.lock`, docs | verify no tooling reads bun.lockb → remove it; pin `packageManager: npm@11.x` via packageManager field; document deno.lock scope; document Node version | OPEN | clean install+build+test | LOW | restore lockfile from git |

### P4 — Supabase client consolidation

| # | Issue | Severity | Root cause | Files affected | Remediation plan | Status | Tests | Regression risk | Rollback |
|---|---|---|---|---|---|---|---|---|---|
| 4-1 | 5 client definitions: canonical `integrations/supabase/client.ts` + aliases `lib/supabase.ts` ≡ `lib/supabaseClient.ts` + dead SSR factories `lib/client.ts`, `lib/server.ts` | MEDIUM | Lovable scaffold + copy-paste aliasing during refactors | those files + orphaned importers (Kargo page, quotation feature, lib/counter.ts) | map importers; migrate/delete after proving unreferenced at runtime; add architecture test forbidding new createClient outside canonical module | OPEN | grep + architecture guard test | LOW (callers are themselves dead-code candidates) | revert |
